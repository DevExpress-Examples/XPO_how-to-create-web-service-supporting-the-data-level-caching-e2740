...
Namespace DXSample.Proxy
    ...
    Public Class CachingDataStore
        ...
        <SoapDocumentMethod("http://devexpress.example/ModifyData", RequestNamespace:="http://devexpress.example/", ResponseNamespace:="http://devexpress.example/")>
        ...
        <SoapDocumentMethod("http://devexpress.example/NotifyDirtyTables", RequestNamespace:="http://devexpress.example/", ResponseNamespace:="http://devexpress.example/")>
        Public Overridable Function NotifyDirtyTables(ByVal cookie As DataCacheCookie, ParamArray dirtyTablesNames As String()) As DataCacheResult
            Return CType(Invoke("NotifyDirtyTables", New Object() {cookie, dirtyTablesNames})(0), DataCacheResult)
        End Function

        <SoapDocumentMethod("http://devexpress.example/ProcessCookie", RequestNamespace:="http://devexpress.example/", ResponseNamespace:="http://devexpress.example/")>
        ...
        <SoapDocumentMethod("http://devexpress.example/SelectData", RequestNamespace:="http://devexpress.example/", ResponseNamespace:="http://devexpress.example/")>
...
#Region "ICacheToCacheCommunicationCore Members"
        ...
        Private Function ICacheToCacheCommunicationCore_NotifyDirtyTables(ByVal cookie As DataCacheCookie, ParamArray dirtyTablesNames As String()) As DataCacheResult Implements ICacheToCacheCommunicationCore.NotifyDirtyTables
...
#Region "IDataStore Members"
        Private ReadOnly Property AutoCreateOption As AutoCreateOption Implements IDataStore.AutoCreateOption
            Get
                Return AutoCreateOption.SchemaAlreadyExists
            End Get
        End Property

        Private Function IDataStore_ModifyData(ParamArray dmlStatements As ModificationStatement()) As ModificationResult Implements IDataStore.ModifyData
            Return ModifyData(DataCacheCookie.Empty, dmlStatements).ModificationResult
        End Function

        Private Function IDataStore_SelectData(ParamArray selects As SelectStatement()) As SelectedData Implements IDataStore.SelectData
            Return SelectData(DataCacheCookie.Empty, selects).SelectedData
        End Function

        Private Function IDataStore_UpdateSchema(ByVal dontCreateIfFirstTableNotExist As Boolean, ParamArray tables As DBTable()) As UpdateSchemaResult Implements IDataStore.UpdateSchema
